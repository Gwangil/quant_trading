...
## 프로젝트가 하는 일
레버리지 ETF 전략(SOXL 분할 바스켓 LOC + GLD 슬리브, docs/00)의 **개발·검증·주문서 발급**. 실제 주문·체결 대사·스케줄 실행은 하지 않는다
(집행기 auto_trade(KIS API) / rpa_claude(Meritz RPA) 가 주문서 파일을 받아서 한다 — docs/05, 역할 경계 docs/07 §1).
성과 판단은 세전 달러 기준.

## 시작할 때 읽을 것
0. `docs/00_strategies.md` — 유효 전략 2개 요약
...
- 새 전략은 `src/qtrade/strategies/` 에 kind 로 등록하고(docs/07 §2), 채택 기준은 바스켓과 동일(IS/OOS, 이웃 절벽 없음, 기존 슬리브와 상관 < 0.5). 기각된 실험 코드는 남기지 않는다 — 결과만 docs/07 §4 와 reports CSV 에.
