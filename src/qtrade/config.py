...
class DataConfig:
    ...
    cache_dir: str = "data/cache"      # source=csv 일 때 읽는 곳 (qtrade data update 가 씀)
    bundled_dir: str = "data/bundled"  # 스냅샷·지수·금리표
    start: str | None = None      # 매매 시작일 (지표 계산은 그 이전 데이터도 사용)
    end: str | None = None             # 매매 종료일 (None = 데이터 끝)
...
@dataclass
class BasketConfig:
    count: int = 4                     # 동시 운용 바스켓 수 = 자산 분할 수
...
class EntryConfig:
    ...
    dip_vol_mult: float = 0.25         # 매수 LOC 지정가 = 전일종가 × (1 − mult × 일변동성)
...
class ExitConfig:
    ...
    soft_exit_pnl_pct: float = 0.0     # max_hold_days 청산 조건이 되는 손익 하한
...
@dataclass
class CostConfig:
    commission_pct: float = 0.001      # 편도 수수료 (0.1%)
...
class StrategyConfig:
    name: str = "basket_loc"           # 리포트·주문서 파일명에 쓰임
    initial_capital: float = 100_000.0 # 초기자본 (USD). 실운용은 계좌 총자산으로 설정
